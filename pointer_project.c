...
int main_pointer_project(void)
{
	...
	long totalEnglishTime = 0; // 총 경과 시간
	...
	while (1)
	{
		...
		// 총 경과 시간
		totalEnglishTime = (clock() - startTime) / CLOCKS_PER_SEC;
		printf("총 경과 시간 : %ld 초\n", totalEnglishTime);

		// 직전 물 준 시간 (마지막으로 물 준 시간) 이후로 흐른 시간
		prevElapsedTime = totalEnglishTime - prevElapsedTime;
		...
		if (totalEnglishTime / 20 > level - 1)
		{	
			// 레벨업
			...
			if (level == 5)
			{
				printf("\n\n 축하합니다. 최고 레벨을 달성하였습니다. 게임을 종료하겠습니다.\n\n");
				exit(0);
			}
		}
		...
		prevElapsedTime = totalEnglishTime;
